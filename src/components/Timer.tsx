...
export default function Timer() {
    ...
    // Parse URL hash parameter for timer value
    useEffect(() => {
        const parseURLHash = () => {
            const hash = window.location.hash;
            if (hash) {
                const match = hash.match(/timer=(\d+)/);
                if (match && match[1]) {
        ...
        // Parse URL when component mounts
        parseURLHash();
    ...
    // Update URL hash when timer changes
    const updateURLHash = (newTime) => {
        window.location.hash = `timer=${newTime}`;
    };
    ...
    // Handle key press for custom time input
    const handleKeyPress = (e) => {
        if (e.key === 'Enter') {
            setCustomTimeHandler();
        }
    };
        ...
        <div className="flex flex-col items-center justify-center p-6 bg-gray-100 min-h-screen">
            <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-6">
                ...
                <div className="mt-6 text-center text-sm text-gray-500">
                    <p>URL Format: /#timer=seconds (e.g., /#timer=4800 for 80 minutes)</p>
                </div>
            </div>
        </div>
    );
